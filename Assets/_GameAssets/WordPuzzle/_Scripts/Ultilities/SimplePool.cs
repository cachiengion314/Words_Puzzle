...
namespace Utilites
{
    public static class SimplePool
    {
        ...
        class Pool
        {
            ...
            public GameObject Spawn(Transform transParent = null)
            {
                GameObject obj;
                if (inactive.Count == 0)
                {
                    ...
                    if(transParent != null)
                        obj = (GameObject)GameObject.Instantiate(prefab,transParent);
                    else
                ...
                else
                {
                    ...
                    if (obj == null)
                    {
                        ...
                        return Spawn(transParent);
                    }
                    else
                    {
                        obj.GetComponent<IPoolItem>()?.Reset();
                    }
                }
        ...
        /// </summary>
        /// 
        static private GameObject Spawn(GameObject prefab, Transform transParent = null)
        {
            Init(prefab);

            return pools[prefab].Spawn(transParent);
        }

        /// <summary>
        /// Despawn the specified gameobject back into its pool.
        /// </summary>
        static private void Despawn(GameObject obj)
        {
        ...
        static public GameObject Spawn(this MonoBehaviour mono, GameObject prefab)
        {
            return Spawn(prefab);
        }

        static public GameObject Spawn(this MonoBehaviour mono, GameObject prefab, Transform transParent)
        {
            var result = Spawn(prefab, transParent);
            return result;
        }

        static public GameObject Spawn(this MonoBehaviour mono, GameObject prefab, Vector3 pos, Quaternion rot)
        {
            var result = Spawn(prefab);
            result.transform.position = pos;
            result.transform.rotation = rot;
            return result;
        }

        static public void Despawn(this MonoBehaviour mono, GameObject obj)
        {
            Despawn(obj);
        }
